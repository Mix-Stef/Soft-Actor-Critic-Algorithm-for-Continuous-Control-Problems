...
class ReplayBuffer:
    def __init__(self, mem_size, num_states, num_actions, batch_size):
        self.mem_size = mem_size
        self.num_states = num_states
        self.num_actions = num_actions
        self.batch_size = batch_size
        assert self.mem_size > self.batch_size, "Batch size cannot be bigger than memory size."
        self.state_mem = np.zeros(shape=(self.mem_size, self.num_states))
        self.action_mem = np.zeros(shape=(self.mem_size, self.num_actions))
